...
//			next_billing_at = this.getDateFormat(next_billing_at);
			...
			String next_billing_at = LocalDateTime.now().plusDays(1).toString();
...
//					.replace("<campaign_id>", camp_id).replace("<type>", "billing")
//					.replace("<sub_date_time>", subscription_at).replace("<result_name>", result_name);

			String instQry = "insert into tbl_dlr (ani,channel_name,status_name,svc_name,amount,campaign_id,next_billed_date,last_billed_date,type,sub_date_time,result_name)"
					+ " values ('" + ani + "','" + channel_name + ">','" + status_name + "','" + svc_name + "','" + amt
					+ "','" + camp_id + "','" + next_billing_at + "','" + last_billed_at + "','billing','"
					+ subscription_at + "','" + result_name + "')";
